...
// Helper to get API key with priority: LocalStorage -> Environment
const getApiKey = (keyName: 'GEMINI_API_KEY' | 'KIEAI_API_KEY'): string => {
  if (typeof window !== 'undefined') {
    const localKey = localStorage.getItem(keyName);
    if (localKey && localKey.trim() !== '') return localKey;
  }
  
  if (keyName === 'GEMINI_API_KEY') return process.env.GEMINI_API_KEY || '';
  if (keyName === 'KIEAI_API_KEY') return process.env.KIEAI_API_KEY || '';
...
export const getVideoFromTaskId = async (taskId: string, onProgress?: (msg: string) => void): Promise<string> => {
  const apiKey = getApiKey('KIEAI_API_KEY');
  if (!apiKey) {
    throw new Error("KIE AI API Key is required. Please set it in Settings.");
  }
